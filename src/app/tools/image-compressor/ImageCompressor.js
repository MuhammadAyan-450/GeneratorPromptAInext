...
import {
  ...
  HardDrive, Percent, Layers, RefreshCw, Zap, Shield, Globe,
  FileText, HelpCircle, Clock, CheckCircle, AlertCircle
} from "lucide-react";
      ...
      <div className="grid md:grid-cols-2 divide-y md:divide-y-0 md:divide-x divide-gray-100">
        ...
        <div className="p-4">
          ...
          <div className="bg-gray-50 rounded-xl overflow-hidden border border-gray-100">
            <img src={item.originalUrl} alt="Original preview" className="w-full h-48 object-contain" />
          </div>
        </div>
        ...
        <div className="p-4">
          <div className="flex justify-between items-center mb-2">
            ...
            {item.compressedSize > 0 && (
              <span className="text-sm font-medium text-green-600">
                ...
                {savings > 0 && (
                  <span className="ml-1 text-xs bg-green-100 text-green-700 px-1.5 py-0.5 rounded-full">-{savings}%</span>
                )}
              </span>
            )}
          </div>
          ...
          {item.loading ? (
            ...
            <>
              <div className="bg-gray-50 rounded-xl overflow-hidden border border-gray-100">
                <img src={item.compressedUrl} alt="Compressed preview" className="w-full h-48 object-contain" />
              </div>
              {savings > 0 && (
                <div className="mt-3">
...
export default function ImageCompressor() {
  ...
  const avgSavings = allDone && images.length
    ? Math.round(images.reduce((acc, img) => {
        const s = img.originalSize > 0 ? ((img.originalSize - img.compressedSize) / img.originalSize) * 100 : 0;
        return acc + s;
      }, 0) / images.length)
    : 0;
  ...
  const addFiles = (files) => {
    ...
    const newImgs = list.map((file) => {
      ...
      return {
        id, file, name: file.name, originalUrl: URL.createObjectURL(file),
        originalSize: file.size, compressedUrl: null, compressedSize: 0,
        loading: true, downloadName: null,
      };
    });
  ...
  const recompressAll = () => {
    setImages((prev) => prev.map((img) => ({ ...img, loading: true, compressedUrl: null, compressedSize: 0 })));
      ...
      <div className="flex-grow max-w-4xl mx-auto w-full px-4 pb-20">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-6 md:p-10 mb-8">
              ...
              {QUALITY_PRESETS.map((preset) => (
                <button
                  ...
                  className={`flex flex-col items-start px-4 py-3 rounded-xl border text-left transition-all ${
                    quality === preset.value ? "border-sky-500 bg-sky-50 text-sky-700" : "border-gray-200 text-gray-600 hover:border-sky-300"
                  }`}
                >
          ...
          {!images.length && (
            <div className="text-center py-16 text-gray-400 border-2 border-dashed border-gray-200 rounded-xl mt-4">
              ...
              <p>Drop your images above or click to browse. We'll compress them instantly — no upload required.</p>
            </div>
          )}
        </div>
        ...
        <section className="bg-white border border-gray-200 rounded-2xl p-6 md:p-10 mb-8">
          ...
          <ol className="space-y-5">
            {[
              { step: "1", title: "Upload Your Images", desc: "Drag and drop JPG, PNG, or WebP files into the upload area. You can add up to 10 images at once for batch compression." },
              { step: "2", title: "Adjust Settings", desc: "Pick a quality preset or fine-tune the slider. Set max dimension for your use case and choose output format if you want to convert." },
              { step: "3", title: "Download Compressed Files", desc: "Wait a few seconds for processing. Preview before/after side by side, then download individually or all at once." },
            ].map((item) => (
        ...
        <section className="bg-white border border-gray-200 rounded-2xl p-6 md:p-10 mb-8">
          ...
          <div className="space-y-5">
            <div className="bg-gray-50 border border-gray-100 rounded-2xl p-5">
              <h3 className="font-bold text-gray-900 text-sm mb-2">Client-Side Processing</h3>
              ...
              <p className="text-gray-500 text-xs leading-relaxed">
                We use the <code className="bg-gray-100 px-1 py-0.5 rounded">browser-image-compression</code> library with Web Workers to handle heavy lifting in the background. Your images never leave your browser.
              </p>
            </div>

            <div className="bg-gray-50 border border-gray-100 rounded-2xl p-5">
              <h3 className="font-bold text-gray-900 text-sm mb-2">Smart Quality Reduction</h3>
              <div className="bg-gray-900 text-green-400 font-mono text-sm px-4 py-3 rounded-xl mb-3 overflow-x-auto">
                quality: 0.80 → ~70% smaller file, same visual quality
              </div>
              <p className="text-gray-500 text-xs leading-relaxed">
                JPEG/WebP compression removes data your eyes barely notice. At 80% quality, most photos look identical but file size drops dramatically.
              </p>
            </div>
            ...
            <div className="bg-sky-50 border border-sky-100 rounded-xl p-4">
              ...
              <p className="text-sky-800 text-xs leading-relaxed">
                All compression happens locally in your browser. No images are uploaded, stored, or sent anywhere. Close the tab and your data is gone. 100% private by design.
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="bg-white border border-gray-200 rounded-2xl p-6 md:p-10 mb-8">
          ...
          <p className="text-gray-500 text-sm mb-6">Real examples with different image types and settings.</p>

          <div className="space-y-5">
            <div className="border border-gray-100 rounded-2xl p-5">
              <div className="flex items-center gap-2 mb-3">
                <span className="text-sm bg-green-100 text-green-700 font-bold px-2.5 py-1 rounded-lg">Photo (JPG)</span>
              </div>
              <div className="grid sm:grid-cols-3 gap-4 text-sm">
                ...
                <div className="bg-gray-50 rounded-lg p-3">
                  ...
                  <p className="font-semibold text-green-600">580 KB <span className="text-xs text-gray-400">(-76%)</span></p>
                </div>
                <div className="bg-gray-50 rounded-lg p-3">
            ...
            <div className="border border-gray-100 rounded-2xl p-5">
              <div className="flex items-center gap-2 mb-3">
                <span className="text-sm bg-blue-100 text-blue-700 font-bold px-2.5 py-1 rounded-lg">Screenshot (PNG)</span>
              </div>
              <div className="grid sm:grid-cols-3 gap-4 text-sm">
                <div className="bg-gray-50 rounded-lg p-3">
                  <p className="text-gray-500 text-xs mb-1">Original</p>
                ...
                <div className="bg-gray-50 rounded-lg p-3">
                  <p className="text-gray-500 text-xs mb-1">Converted to WebP (80%)</p>
                  <p className="font-semibold text-green-600">420 KB <span className="text-xs text-gray-400">(-77%)</span></p>
                </div>
                <div className="bg-gray-50 rounded-lg p-3">
                  <p className="text-gray-500 text-xs mb-1">Best For</p>
                  <p className="font-semibold text-gray-800">WebP-supported sites</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <section className="bg-white border border-gray-200 rounded-2xl p-6 md:p-10 mb-8">
          ...
          <p className="text-gray-500 text-sm mb-6">From bloggers to businesses — fast images help everyone.</p>
          <div className="grid sm:grid-cols-2 gap-4">
            {[
              { icon: <Globe size={20} className="text-sky-600" />, title: "Website Owners", desc: "Speed up page load times by compressing hero images, product photos, and blog graphics before uploading." },
              { icon: <FileText size={20} className="text-green-600" />, title: "Content Creators", desc: "Reduce image size for email newsletters, social media posts, and WhatsApp without losing visual quality." },
              { icon: <Zap size={20} className="text-amber-600" />, title: "E-commerce Sellers", desc: "Compress product images to improve site performance and mobile shopping experience for customers." },
              { icon: <Shield size={20} className="text-violet-600" />, title: "Privacy-Conscious Users", desc: "Compress personal photos without uploading to third-party servers. Everything stays on your device." },
            ].map((item, i) => (
        ...
        <section className="bg-white border border-gray-200 rounded-2xl p-6 md:p-10 mb-8">
          ...
          <p className="text-gray-600 mb-4 leading-relaxed">
            Large images slow down websites, eat up mobile data, and sometimes fail to send over email or messaging apps. Our free image compressor solves this by shrinking JPG, PNG, and WebP files directly in your browser — no upload required.
          </p>
          <p className="text-gray-600 mb-4 leading-relaxed">
            Most photos can be reduced by 60-80% with barely any visible difference. Pick the <strong>Balanced (80%)</strong> preset for the best mix of quality and savings. Need smaller files for WhatsApp? Try <strong>Aggressive (65%)</strong> with 1200px max dimension.
          </p>
          <p className="text-gray-600 mb-4 leading-relaxed">
            Converting to <strong>WebP</strong> while compressing gives you even more savings — typically 25-35% smaller than JPEG at the same quality. Modern browsers like Chrome, Firefox, Safari, and Edge all support WebP now.
          </p>
          ...
          <p className="text-gray-600 leading-relaxed mt-4">
            ...
            <Link href="/tools/image-resizer" className="text-sky-600 underline underline-offset-2 hover:text-sky-700">Image Resizer</Link>. 
        ...
        <section className="bg-white border border-gray-200 rounded-2xl p-6 md:p-10 mb-8">
          ...
          <div className="space-y-3 max-w-4xl mx-auto">
            {[
              { q: "How to compress image without losing quality online for free?", a: "Upload your image and select the Maximum (92%) or Balanced (80%) quality preset. The tool compresses in your browser without uploading to any server, preserving visual quality while reducing file size significantly." },
              { q: "Does this image compressor upload my photos to a server?", a: "No. Our tool works 100% in your browser using JavaScript. Your images never leave your device. No data is uploaded, stored, or sent to any server." },
              { q: "How to reduce JPG file size for WhatsApp without losing quality?", a: "Upload your JPG photo, select Balanced (80%) quality and 1200px max dimension, then compress and download. The result will be a much smaller file that WhatsApp sends instantly without visible quality loss." },
              { q: "Can I batch compress multiple images at once for free?", a: "Yes. You can upload up to 10 images at once and they will all be compressed simultaneously with the same quality and dimension settings. Use the Download All button to save them all at once." },
              { q: "How to compress PNG to WebP for faster website loading?", a: "Upload your PNG images, select WebP as the output format, choose Balanced or Aggressive quality, and compress. WebP produces files 25-35% smaller than PNG at the same visual quality and is supported by all modern browsers." },
              { q: "How much can I reduce image file size?", a: "Depending on the original image and settings, you can reduce file size by 50-90%. Most photos compress 60-80% with the Balanced (80%) preset while maintaining excellent visual quality." },
            ].map((item, i) => (
        ...
        <section>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              { href: "/tools/image-resizer", title: "Image Resizer", desc: "Resize photos to exact dimensions for any platform." },

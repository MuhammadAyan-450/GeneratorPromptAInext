...
export default function ImageCompressor() {
  ...
  const recompressAll = () => {
    setImages((prev) =>
      prev.map((img) => ({ ...img, loading: true, compressedUrl: null, compressedSize: 0 }))
    );
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="flex-grow max-w-4xl mx-auto w-full px-4 pb-20">
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-6 md:p-10 mb-8">
          ...
          <p className="text-gray-600 mb-4 leading-relaxed">
            Unlike other tools that upload your photos to a remote server for processing, our compressor uses the <code className="bg-gray-100 px-1.5 py-0.5 rounded text-sm">browser-image-compression</code> library to do everything locally. This means your images are <strong>100% private</strong> — they never leave your device.
          </p>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-6 md:p-10 mb-8">
          ...
          <div className="grid md:grid-cols-2 gap-5">
            {[
              { icon: <Shield size={20} className="text-sky-600" />, title: "100% Browser-Based Privacy", desc: "No images are uploaded to any server. All compression happens locally in your browser using JavaScript. Your photos stay on your device at all times." },
              { icon: <Layers size={20} className="text-green-600" />, title: "Batch Compress Up to 10 Images", desc: "Upload multiple images at once and compress them all simultaneously with the same quality and dimension settings. Download individually or all at once." },
              { icon: <Zap size={20} className="text-amber-500" />, title: "Format Conversion While Compressing", desc: "Convert PNG to WebP, JPG to WebP, or any combination while compressing. WebP produces files 25-35% smaller than JPEG at the same visual quality." },
              { icon: <HardDrive size={20} className="text-violet-600" />, title: "Visual Before/After Comparison", desc: "See original and compressed images side by side with exact file sizes and percentage saved. Know exactly what you are getting before downloading." }
            ].map((feature, i) => (

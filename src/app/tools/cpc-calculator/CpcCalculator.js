...
import {
  ...
  Zap, Shield, Target, CheckCircle2, TrendingUp, Code, HelpCircle, ArrowRight, FileText
} from "lucide-react";
      ...
      <div className="max-w-4xl mx-auto w-full px-4 pt-6">
        <nav aria-label="Breadcrumb">
          <ol className="flex items-center gap-2 text-sm text-gray-500">
            ...
            <li><span className="text-gray-300">/</span></li>
            <li><span className="text-gray-900 font-semibold">CPC Calculator</span></li>
          </ol>
        </nav>
      </div>
      ...
      <div className="flex-grow max-w-4xl mx-auto w-full px-4 pb-20">
        ...
        <div className="text-center mb-10 mt-4">
          ...
          <p className="text-gray-500 text-base md:text-lg max-w-2xl mx-auto">
            Calculate cost per click, campaign cost, or total clicks instantly. Just enter two values, hit calculate, and get your CPC metrics plus bonus CTR and CPM data.
          </p>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-6 md:p-10 mb-8">
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            {mode === "cpc" && (
              <>
                <div>
                  ...
                  <input type="number" step="0.01" placeholder="e.g. 500" value={cost} onChange={(e) => setCost(e.target.value)} className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent text-gray-800" />
                </div>
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">Total Clicks</label>
                  <input type="number" placeholder="e.g. 100" value={clicks} onChange={(e) => setClicks(e.target.value)} className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent text-gray-800" />
                </div>
              </>
            )}
            ...
            {mode === "cost" && (
              <>
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">CPC Rate ($)</label>
                  <input type="number" step="0.01" placeholder="e.g. 2.50" value={cpc} onChange={(e) => setCpc(e.target.value)} className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent text-gray-800" />
                </div>
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">Total Clicks</label>
                  <input type="number" placeholder="e.g. 1000" value={clicks} onChange={(e) => setClicks(e.target.value)} className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent text-gray-800" />
                </div>
              </>
            )}
          ...
          <div className="mb-6">
            <label className="block text-sm font-semibold text-gray-700 mb-2">Total Impressions (Optional)</label>
            <input type="number" placeholder="e.g. 10000" value={impressions} onChange={(e) => setImpressions(e.target.value)} className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent text-gray-800" />
            <p className="text-xs text-gray-500 mt-1">Add impressions to auto-calculate CTR & CPM.</p>
          </div>

          {/* Action Buttons */}
          <div className="grid grid-cols-2 gap-4 mb-6">
            <button onClick={calculate} disabled={isDisabled} className="bg-sky-600 hover:bg-sky-700 disabled:opacity-40 text-white font-semibold py-3 rounded-xl transition-all shadow-sm hover:shadow-md flex items-center justify-center gap-2">
              <Calculator size={18} /> Calculate
            </button>
            <button onClick={handleReset} className="bg-white border-2 border-sky-100 text-sky-700 hover:bg-sky-50 font-semibold py-3 rounded-xl transition-all flex items-center justify-center gap-2">
              Clear All
            </button>
          </div>
          ...
          {result && (
            <div className="bg-sky-50 border border-sky-100 rounded-xl p-5 mb-6">
              <p className="text-xs font-semibold text-sky-500 uppercase tracking-wider mb-3">{result.label}</p>
          ...
          <div className="flex flex-wrap justify-center gap-3 border-t border-gray-100 pt-6">
            ...
            <button onClick={handleReset} className="inline-flex items-center gap-2 px-5 py-2.5 bg-white border border-gray-300 hover:bg-gray-50 text-red-600 rounded-xl text-sm font-semibold transition-all">
        ...
        <section className="bg-white border border-gray-200 rounded-2xl p-6 md:p-10 mb-8">
          ...
          <p className="text-gray-500 text-sm mb-6">It's simple math. Here's exactly what happens when you click Calculate.</p>

          <div className="space-y-5">
            <div className="bg-gray-50 border border-gray-100 rounded-2xl p-5">
              <h3 className="font-bold text-gray-900 text-sm mb-2">Calculate CPC</h3>
              <div className="bg-gray-900 text-green-400 font-mono text-sm px-4 py-3 rounded-xl mb-3 overflow-x-auto">
                CPC = Total Cost ÷ Total Clicks
              </div>
              <p className="text-gray-500 text-xs leading-relaxed">Divides your total spend by the number of clicks received. Example: $500 / 100 clicks = $5.00 CPC.</p>
            </div>

            <div className="bg-gray-50 border border-gray-100 rounded-2xl p-5">
              <h3 className="font-bold text-gray-900 text-sm mb-2">Calculate Total Cost</h3>
              <div className="bg-gray-900 text-green-400 font-mono text-sm px-4 py-3 rounded-xl mb-3 overflow-x-auto">
                Total Cost = CPC × Total Clicks
              </div>
              <p className="text-gray-500 text-xs leading-relaxed">Multiplies your target CPC by desired clicks to find budget. Example: $2.50 × 1000 clicks = $2,500 budget.</p>
            </div>

            <div className="bg-gray-50 border border-gray-100 rounded-2xl p-5">
              <h3 className="font-bold text-gray-900 text-sm mb-2">Calculate Total Clicks</h3>
              <div className="bg-gray-900 text-green-400 font-mono text-sm px-4 py-3 rounded-xl mb-3 overflow-x-auto">
                Total Clicks = Total Cost ÷ CPC
              </div>
              <p className="text-gray-500 text-xs leading-relaxed">Divides your budget by CPC to estimate traffic. Example: $500 / $2.50 CPC = 200 clicks.</p>
            </div>

            <div className="bg-sky-50 border border-sky-100 rounded-xl p-4">
              <h3 className="font-bold text-sky-900 text-sm mb-2">Bonus Metrics (with Impressions)</h3>
              <div className="bg-gray-900 text-green-400 font-mono text-sm px-4 py-3 rounded-xl overflow-x-auto">
                CTR = (Clicks ÷ Impressions) × 100<br/>
        ...
        <section className="bg-white border border-gray-200 rounded-2xl p-6 md:p-10 mb-8">
          ...
          <p className="text-gray-500 text-sm mb-6">Real-world scenarios to help you understand the metrics.</p>
          ...
          <div className="space-y-5">
            <div className="border border-gray-100 rounded-2xl p-5">
              ...
              <div className="bg-green-50 border border-green-100 rounded-lg p-4">
                ...
                <p className="font-mono text-sm text-gray-800">Cost: $500, Clicks: 100</p>
                <p className="text-xs text-green-600 font-mono break-all mt-2">Output:</p>
                <p className="font-mono text-sm text-gray-800 break-all">CPC: $5.00</p>
              </div>
            </div>
            ...
            <div className="border border-gray-100 rounded-2xl p-5">
              ...
              <div className="bg-green-50 border border-green-100 rounded-lg p-4">
                <p className="text-xs text-green-700 font-medium mb-1">Input:</p>
                <p className="font-mono text-sm text-gray-800">CPC: $2.50, Clicks: 1000, Impressions: 50,000</p>
                <p className="text-xs text-green-600 font-mono break-all mt-2">Output:</p>
                <p className="font-mono text-sm text-gray-800 break-all">Total Cost: $2,500.00, CTR: 2.00%, CPM: $50.00</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="bg-white border border-gray-200 rounded-2xl p-6 md:p-10 mb-8">
          ...
          <p className="text-gray-500 text-sm mb-6">Not just marketers. Here's where CPC metrics show up in real work.</p>
        ...
        <section className="bg-white border border-gray-200 rounded-2xl p-6 md:p-10 mb-8">
          ...
          <p className="text-gray-600 mb-4 leading-relaxed">
            <strong>CPC (Cost Per Click)</strong> is the amount you pay each time a user clicks on your digital ad. It is the most important metric for pay-per-click (PPC) campaigns on platforms like Google Ads, Facebook Ads, and LinkedIn Ads.
          </p>
          <p className="text-gray-600 mb-4 leading-relaxed">
            Knowing your CPC helps you plan budgets. If your target CPC is $3 and you want 500 clicks, your budget should be $3 × 500 = $1,500. This calculator handles all three directions instantly.
          </p>
          <p className="text-gray-600 mb-4 leading-relaxed">
          ...
          <p className="text-gray-600 mb-4 leading-relaxed">
            High CPC is usually caused by bidding on competitive short-tail keywords, low Quality Score, poor landing page experience, or targeting high-competition audiences. Improve your ad relevance, use long-tail keywords, and optimize your landing pages to lower costs.
          </p>
          ...
          <p className="text-gray-600 leading-relaxed">
            This tool runs 100% in your browser. Your data never leaves your device — no server upload, no logging, no tracking. Close the tab and it's gone. That's how it should be.
          </p>

          <p className="text-gray-600 leading-relaxed mt-4">
            Need to calculate CPM instead? Try the{" "}
            <Link href="/tools/cpm-calculator" className="text-sky-600 underline underline-offset-2 hover:text-sky-700">CPM Calculator</Link>. Estimating overall revenue? The{" "}
            <Link href="/tools/ad-revenue-calculator" className="text-sky-600 underline underline-offset-2 hover:text-sky-700">Ad Revenue Calculator</Link> has your back.
          </p>
        </section>
        ...
        <section>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              { href: "/tools/cpm-calculator", title: "CPM Calculator", desc: "Calculate cost per 1000 impressions." },
              { href: "/tools/ad-revenue-calculator", title: "Ad Revenue Calculator", desc: "Estimate income from display ads." },
              { href: "/tools/percentage-calculator", title: "Percentage Calculator", desc: "Calculate increases and decreases." },
              { href: "/tools/adsense-revenue-calculator", title: "AdSense Revenue Calculator", desc: "Estimate Google AdSense earnings." },
              { href: "/tools/youtube-ad-revenue-calculator", title: "YouTube Revenue Calculator", desc: "Estimate YouTube ad income." },

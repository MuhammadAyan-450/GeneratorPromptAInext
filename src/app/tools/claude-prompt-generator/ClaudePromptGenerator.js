...
import {
  Copy, RefreshCw, Star, Trash2, Sparkles, ExternalLink, Home, ChevronDown,
  Code, HelpCircle, ArrowRight, Zap, Shield, UserCheck, FileText
} from "lucide-react";
      ...
      <div className="max-w-4xl mx-auto w-full px-4 pt-6">
        <nav aria-label="Breadcrumb">
          <ol className="flex items-center gap-2 text-sm text-gray-500">
            ...
            <li><span className="text-gray-300">/</span></li>
            <li><span className="text-gray-900 font-semibold">Claude Prompt Generator</span></li>
          </ol>
        </nav>
      </div>
      ...
      <div className="flex-grow max-w-4xl mx-auto w-full px-4 pb-20">
        ...
        <div className="text-center mb-10 mt-4">
          ...
          <p className="text-gray-500 text-base md:text-lg max-w-2xl mx-auto">
            Claude AI responds differently than other models. It thrives on structured instructions and explicit roles. Instead of guessing, just enter your topic, pick a use case, and let this tool structure the perfect request for you.
          </p>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-6 md:p-10 mb-8">
          ...
          {/* Action Buttons */}
          <div className="grid grid-cols-2 gap-4 mb-6">
            <button onClick={generatePrompt} className="bg-orange-500 hover:bg-orange-600 text-white font-semibold py-3 rounded-xl transition-all shadow-sm hover:shadow-md flex items-center justify-center gap-2">
              <Sparkles size={18} /> Generate
            </button>
            <button onClick={clearAll} className="bg-white border-2 border-orange-100 text-orange-700 hover:bg-orange-50 font-semibold py-3 rounded-xl transition-all flex items-center justify-center gap-2">
              Clear All
            </button>
          </div>
          ...
          {result && (
            <div className="bg-orange-50 border border-orange-100 rounded-xl p-5 mb-6">
              <p className="text-xs font-semibold text-orange-500 uppercase tracking-wider mb-3">Generated Prompt</p>
              <p className="whitespace-pre-wrap text-gray-800 leading-relaxed text-sm">{result}</p>
              <div className="flex flex-wrap gap-3 mt-4">
                <button onClick={copyPrompt} className="inline-flex items-center gap-2 px-5 py-2.5 bg-orange-500 hover:bg-orange-600 text-white rounded-xl text-sm font-semibold transition-all shadow-sm hover:shadow-md">
                  <Copy size={15} /> {copied ? "Copied!" : "Copy"}
                </button>
                <button onClick={openInClaude} className="inline-flex items-center gap-2 px-5 py-2.5 bg-gray-900 hover:bg-gray-800 text-white rounded-xl text-sm font-semibold transition-all">
                  <ExternalLink size={15} /> Open in Claude
                </button>
                <button onClick={addToFavorites} className="inline-flex items-center gap-2 px-5 py-2.5 bg-yellow-100 hover:bg-yellow-200 text-yellow-800 rounded-xl text-sm font-semibold transition-all">
          ...
          <div className="flex flex-wrap justify-center gap-3 border-t border-gray-100 pt-6">
            ...
            <button onClick={addToFavorites} className="inline-flex items-center gap-2 px-5 py-2.5 bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 rounded-xl text-sm font-semibold transition-all">
              <Star size={16} /> Save to Favorites
            </button>
            <button onClick={clearAll} className="inline-flex items-center gap-2 px-5 py-2.5 bg-white border border-gray-300 hover:bg-gray-50 text-red-600 rounded-xl text-sm font-semibold transition-all">
        ...
        <section className="bg-white border border-gray-200 rounded-2xl p-6 md:p-10 mb-8">
          ...
          <p className="text-gray-500 text-sm mb-6">It's not magic — just structured instructions. Here's what happens when you click Generate.</p>

          <div className="space-y-5">
            <div className="bg-gray-50 border border-gray-100 rounded-2xl p-5">
              <h3 className="font-bold text-gray-900 text-sm mb-2">Base Template Selection</h3>
              ...
              <p className="text-gray-500 text-xs leading-relaxed">Picks one of 9 proven prompt templates based on your use case — coding, writing, analysis, etc.</p>
            </div>

            <div className="bg-gray-50 border border-gray-100 rounded-2xl p-5">
              <h3 className="font-bold text-gray-900 text-sm mb-2">Topic Injection</h3>
              <div className="bg-gray-900 text-green-400 font-mono text-sm px-4 py-3 rounded-xl mb-3 overflow-x-auto">
                prompt = base.replaceAll("{'{'}topic{'}'}", userTopic);
              </div>
              <p className="text-gray-500 text-xs leading-relaxed">Replaces the {'{'}topic{'}'} placeholder with your actual input. Safe string replacement — no code injection risks.</p>
            </div>

            <div className="bg-gray-50 border border-gray-100 rounded-2xl p-5">
              <h3 className="font-bold text-gray-900 text-sm mb-2">Use Case Role Assignment</h3>
              <div className="bg-gray-900 text-green-400 font-mono text-sm px-4 py-3 rounded-xl mb-3 overflow-x-auto">
                if (useCase === "coding") prompt += " You are an expert software engineer...";
              </div>
              <p className="text-gray-500 text-xs leading-relaxed">Assigns a specific expert role to Claude — software engineer, writer, analyst — so it responds with the right expertise.</p>
            </div>

            <div className="bg-gray-50 border border-gray-100 rounded-2xl p-5">
              <h3 className="font-bold text-gray-900 text-sm mb-2">XML Tags for Native Format</h3>
              <div className="bg-gray-900 text-green-400 font-mono text-sm px-4 py-3 rounded-xl mb-3 overflow-x-auto">
                if (format === "xml") prompt += " Use {'<'}thinking{'>'} for reasoning and {'<'}output{'>'} for answer.";
              </div>
              <p className="text-gray-500 text-xs leading-relaxed">Adds Claude's native XML tags to separate reasoning from final answer — dramatically improves accuracy on complex tasks.</p>
            </div>

            <div className="bg-orange-50 border border-orange-100 rounded-xl p-4">
              <h3 className="font-bold text-orange-900 text-sm mb-2">Full Formula</h3>
        ...
        <section className="bg-white border border-gray-200 rounded-2xl p-6 md:p-10 mb-8">
          ...
          <p className="text-gray-500 text-sm mb-6">These are real outputs from the tool. Try them yourself above.</p>

          <div className="space-y-5">
            <div className="border border-gray-100 rounded-2xl p-5">
              <div className="flex items-center gap-2 mb-3">
                <span className="text-sm bg-orange-100 text-orange-700 font-bold px-2.5 py-1 rounded-lg">Input</span>
              </div>
              <div className="bg-green-50 border border-green-100 rounded-lg p-4">
                <p className="text-xs text-green-700 font-medium mb-1">Topic:</p>
                <p className="font-mono text-sm text-gray-800">sustainable fashion</p>
                <p className="text-xs text-green-600 font-mono break-all mt-2">Use Case: Writing, Depth: Comprehensive, Tone: Professional, Format: Markdown</p>
              </div>
            </div>

            <div className="border border-gray-100 rounded-2xl p-5">
              <div className="flex items-center gap-2 mb-3">
                <span className="text-sm bg-orange-100 text-orange-700 font-bold px-2.5 py-1 rounded-lg">Output</span>
              </div>
              <div className="bg-green-50 border border-green-100 rounded-lg p-4">
                <p className="font-mono text-sm text-gray-800 break-all">
                  Create a detailed blog post about sustainable fashion with headings, subheadings, and practical tips. You are a skilled writer and editor. Focus on clarity, engagement, strong structure, and compelling language. Deliver a comprehensive, in-depth answer with step-by-step reasoning, real-world examples, pros/cons, edge cases, and practical applications. Use a polished, professional, confident tone suitable for business or technical contexts. Format your response using rich markdown: use ## headings, **bold** for key terms, bullet lists, and code blocks where relevant.
                </p>
              </div>
            </div>
            ...
            <div className="border border-gray-100 rounded-2xl p-5">
              <div className="flex items-center gap-2 mb-3">
                <span className="text-sm bg-orange-100 text-orange-700 font-bold px-2.5 py-1 rounded-lg">Another Example</span>
              </div>
              <div className="bg-green-50 border border-green-100 rounded-lg p-4">
                <p className="text-xs text-green-700 font-medium mb-1">Topic:</p>
                <p className="font-mono text-sm text-gray-800">Python list comprehensions</p>
                <p className="text-xs text-green-600 font-mono break-all mt-2">Use Case: Coding, Depth: Expert, Tone: Academic, Format: XML</p>
                <p className="text-xs text-green-600 font-mono break-all mt-3">Output:</p>
                <p className="font-mono text-sm text-gray-800 break-all">
                  Act as an expert and explain Python list comprehensions in extreme detail with step-by-step reasoning. You are an expert software engineer. Focus on clean, efficient, well-commented code with error handling and best practices. Act as a world-class domain expert. Provide advanced, nuanced insights including relevant frameworks, case studies, potential pitfalls, and actionable strategies. Use precise, professional language. Structure your response using Claude's native XML tags. Use {'<'}thinking{'>'} for your reasoning process and {'<'}output{'>'} for the final answer.
                </p>
              </div>
            </div>
          </div>
        ...
        <section className="bg-white border border-gray-200 rounded-2xl p-6 md:p-10 mb-8">
          ...
          <p className="text-gray-500 text-sm mb-6">Not just developers. Here's where structured prompts show up in real work.</p>
        ...
        <section className="bg-white border border-gray-200 rounded-2xl p-6 md:p-10 mb-8">
          ...
          <p className="text-gray-600 mb-4 leading-relaxed">
            There's a reason some people get amazing results from Claude while others get generic fluff. It's not the AI — it's the prompt.
          </p>
          <p className="text-gray-600 mb-4 leading-relaxed">
            Vague prompts get vague answers. "Tell me about marketing" gives you a Wikipedia summary. "Write a LinkedIn post about my experience with B2B SaaS marketing, professional tone, include 3 actionable tips" gives you publish-ready content.
          </p>
          <p className="text-gray-600 mb-4 leading-relaxed">
            This tool removes the guesswork. You focus on your topic. It handles the prompt engineering — use cases, depth levels, tone modifiers, XML tags — so you get better responses without learning prompt syntax.
          </p>
          ...
          <p className="text-gray-600 mb-4 leading-relaxed">
            Claude responds best when you separate its thinking from its output. Using {'<'}thinking{'>'} for reasoning and {'<'}output{'>'} for the final answer dramatically improves accuracy on complex tasks. This tool adds these tags automatically when you select the XML format.
          </p>
          ...
          <p className="text-gray-600 mb-4 leading-relaxed">
            Your prompt needs a role. "You are an expert software engineer" gets better code than "Write some code". "You are a senior business consultant" gets better strategy than "Give me business advice". This tool assigns the right role automatically.
          </p>
          ...
          <p className="text-gray-600 leading-relaxed">
            This tool runs 100% in your browser. Your topics and prompts never leave your device — no server upload, no logging, no tracking. Close the tab and it's gone. That's how it should be.
          </p>

          <p className="text-gray-600 leading-relaxed mt-4">
            Need prompts for ChatGPT instead? Try the{" "}
            <Link href="/tools/chatgpt-prompt-generator" className="text-orange-500 underline underline-offset-2 hover:text-orange-600">ChatGPT Prompt Generator</Link>. Working on video scripts? The{" "}
            <Link href="/tools/youtube-script-prompt-generator" className="text-orange-500 underline underline-offset-2 hover:text-orange-600">YouTube Script Prompt Builder</Link> has your back.
          </p>
        </section>
        ...
        <section>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              { href: "/tools/chatgpt-prompt-generator", title: "ChatGPT Prompt Generator", desc: "Optimized prompts for OpenAI's GPT-4 models." },
              { href: "/tools/midjourney-prompt-generator", title: "Midjourney Prompt Builder", desc: "Craft detailed image prompts with styles." },
              { href: "/tools/youtube-script-prompt-generator", title: "YouTube Script Prompt Builder", desc: "Generate video script prompts and outlines." },
              { href: "/tools/ai-text-humanizer", title: "AI Text Humanizer", desc: "Convert AI text to sound more human-written." },
              { href: "/tools/ai-content-detector", title: "AI Content Detector", desc: "Check if text was written by AI or human." },
              { href: "/tools/word-counter", title: "Word Counter", desc: "Count words, characters, and reading time." },
            ].map((tool) => (
              <Link key={tool.href} href={tool.href} className="group bg-white border border-gray-200 rounded-xl p-5 hover:shadow-md hover:border-orange-400 transition-all">

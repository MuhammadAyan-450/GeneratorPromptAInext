...
import {
  Copy, RefreshCw, Home, ChevronDown, Download, DollarSign, Play, Zap, Shield, Eye, Clock, CheckCircle2, TrendingUp, Globe, HelpCircle, FileText, BarChart3
} from 'lucide-react';
...
const calculateRevenue = (views, cpm, videoLength, locationTier, selectedCategories) => {
...
const YoutubeAdRevenueCalculator = () => {
  ...
  const handleCalculate = () => {
    ...
    const results = calculateRevenue(views, cpm, videoLength, locationTier, selectedCategories);
    ...
    setError('');
    setResult({ results, grandMonthly: grandMonthly.toFixed(2), grandYearly: grandYearly.toFixed(2), monetizedViews });
    setCopied(false);
  };
  ...
  const stats = result
    ? [
        { icon: Eye, label: 'Monetized Views', value: result.monetizedViews.toLocaleString(), color: 'text-gray-800' },
        { icon: DollarSign, label: 'Avg CPM', value: `$${(result.results.reduce((sum, r) => sum + parseFloat(r.effectiveCpm), 0) / result.results.length).toFixed(2)}`, color: 'text-sky-600' },
        { icon: TrendingUp, label: 'Monthly Est.', value: `$${result.grandMonthly}`, color: 'text-green-600' },
        { icon: BarChart3, label: 'Yearly Est.', value: `$${result.grandYearly}`, color: 'text-indigo-600' },
      ]
    : [];
      ...
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div className="bg-white border border-gray-200 rounded-3xl shadow-sm p-6 md:p-10 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            <button onClick={handleCalculate} disabled={!views || !cpm || selectedCategories.length === 0} className="bg-sky-600 hover:bg-sky-700 disabled:opacity-40 disabled:cursor-not-allowed text-white font-semibold py-3.5 rounded-xl transition-all shadow-sm hover:shadow-md flex items-center justify-center gap-2.5">
              <TrendingUp size={18} /> Calculate YouTube Revenue
            </button>
            <button onClick={resetAll} className="bg-white border-2 border-sky-200 text-sky-700 hover:bg-sky-50 font-semibold py-3.5 rounded-xl transition-all flex items-center justify-center gap-2.5">
        ...
        <section className="bg-white border border-gray-200 rounded-3xl p-6 md:p-10 mb-8">
          ...
          <ol className="space-y-5">
            {[
              ...
              { step: '3', title: 'Configure Settings', desc: 'Video length determines mid-roll eligibility. Location tier adjusts CPM based on audience geography.' },
              { step: '4', title: 'Select Categories', desc: 'Choose video niches to see estimated revenue for each side by side.' },
            ].map((item) => (
        ...
        {/* ── How It Works ── */}
        <section className="bg-white border border-gray-200 rounded-3xl p-6 md:p-10 mb-8">
          <h2 className="text-2xl font-bold text-gray-900 mb-2">How YouTube Revenue Calculation Works</h2>
          <p className="text-gray-500 text-sm mb-6">Smart estimation, real-world factors. Here's the logic.</p>
          ...
          <div className="space-y-5">
            <div className="bg-gray-50 border border-gray-100 rounded-2xl p-5">
              ...
              <p className="text-gray-500 text-xs leading-relaxed">
                Not all views generate ad revenue. We assume a 55% monetization rate to account for ad blockers, YouTube Premium users, and skipped ads.
              </p>
            </div>
            ...
            <div className="bg-gray-50 border border-gray-100 rounded-2xl p-5">
              ...
              <p className="text-gray-500 text-xs leading-relaxed">
                Finance/Tech niches have higher CPMs. Tier 1 countries pay more. Videos over 8 minutes get a 1.5x boost for mid-roll ads.
              </p>
            </div>
            ...
            <div className="bg-sky-50 border border-sky-100 rounded-xl p-4">
              ...
              <p className="text-sky-800 text-xs leading-relaxed">
                All calculations happen locally in your browser. No channel data is ever sent to any server. 100% private.
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="bg-white border border-gray-200 rounded-3xl p-6 md:p-10 mb-8">
          ...
          <p className="text-gray-500 text-sm mb-6">See how different factors impact revenue.</p>
            ...
            <div className="grid sm:grid-cols-2 gap-4 mb-4">
              <div className="bg-white border border-gray-200 rounded-xl p-4">
                ...
                <p className="font-mono text-xs text-gray-800">
                  Views: 1,000,000<br />
                  Base CPM: $5.00<br />
                  Category: Technology (1.5x)<br />
                  Location: Tier 1 (1.0x)<br />
              ...
              <div className="bg-white border border-gray-200 rounded-xl p-4">
                ...
                <p className="font-mono text-xs text-gray-800">
                  Effective CPM: $11.25<br />
                  Monetized Views: 550,000<br />
                  Monthly Revenue: $6,187.50<br />
                  Yearly Revenue: $74,250.00
                </p>
              </div>
            </div>
            <p className="text-gray-600 text-sm leading-relaxed">
              Result: High-value niche + Tier 1 audience + mid-rolls = significantly higher earnings than average.
            </p>
          </div>
        </section>
        ...
        <section className="bg-white border border-gray-200 rounded-3xl p-6 md:p-10 mb-8">
          ...
          <p className="text-gray-500 text-sm mb-6">From creators to agencies — accurate estimates drive strategy.</p>
          <div className="grid sm:grid-cols-2 gap-4">
            {[
              { icon: <Zap size={20} className="text-sky-600" />, title: 'YouTubers & Creators', desc: 'Forecast monthly income based on view trends and niche performance to plan content strategy.' },
              { icon: <BarChart3 size={20} className="text-sky-600" />, title: 'MCNs & Agencies', desc: 'Evaluate potential channel acquisitions by estimating ad revenue across different geographies.' },
              { icon: <Globe size={20} className="text-sky-600" />, title: 'Brand Marketers', desc: 'Understand creator earnings to negotiate fair sponsorship deals relative to ad revenue potential.' },
              { icon: <HelpCircle size={20} className="text-sky-600" />, title: 'Aspiring Creators', desc: 'Set realistic income goals by understanding how niche, location, and video length impact earnings.' },
            ].map((item, i) => (
        ...
        <section className="bg-white border border-gray-200 rounded-3xl p-6 md:p-10 mb-8">
          ...
          <p className="text-gray-600 mb-4 leading-relaxed">
            This YouTube ad revenue calculator helps creators estimate monthly and yearly income from YouTube ads based on views, CPM, video category, length, and audience geography. It accounts for the fact that only ~55% of views are typically monetized due to ad blockers and YouTube Premium.
          </p>
          <p className="text-gray-600 mb-4 leading-relaxed">
            Finance and investing channels typically earn the highest CPMs ($15-30), while music and entertainment channels have lower CPMs ($1-5) but can compensate with high view volumes. Videos over 8 minutes unlock mid-roll ads which can increase revenue by 50-100%. Our calculator factors in all these variables automatically.
          </p>
          <p className="text-gray-600 leading-relaxed">
            Built for speed and privacy — all calculations happen in your browser using JavaScript. Your channel data never leaves your device. Just accurate estimates, instantly.
          </p>

          <h3 className="text-lg font-bold text-gray-900 mb-3 mt-8">Privacy Note</h3>
          <p className="text-gray-600 leading-relaxed">
            This tool runs 100% in your browser using JavaScript. No channel data is uploaded to any server. No data is stored or tracked. Your info stays on your device — always.
          </p>

          <p className="text-gray-600 leading-relaxed mt-4">
            Need more creator tools? Try the{' '}
            <Link href="/tools/adsense-revenue-calculator" className="text-sky-600 underline underline-offset-2 hover:text-sky-700">AdSense Revenue Calculator</Link> for blog earnings, or the{' '}
            <Link href="/tools/cpm-calculator" className="text-sky-600 underline underline-offset-2 hover:text-sky-700">CPM Calculator</Link> for general ad metrics.
          </p>
        </section>

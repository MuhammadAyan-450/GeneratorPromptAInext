...
import {
  Copy, RefreshCw, Hash, Home, ChevronDown, Zap, BarChart2, Target, Clock,
  Code, HelpCircle, Shield, Globe, FileText, Download
} from "lucide-react";
...
function buildHashtags(words, language, maxLength) {
  ...
  const mods = language === "urdu-roman" ? [...MODIFIERS.trending, ...MODIFIERS.desi] : language === "mixed" ? [...MODIFIERS.trending, ...MODIFIERS.common, ...MODIFIERS.desi, ...MODIFIERS.niche] : [...MODIFIERS.trending, ...MODIFIERS.common, ...MODIFIERS.niche];
...
const HashtagGenerator = () => {
  ...
  const generateHashtags = () => {
    ...
    const words = keywords.toLowerCase().split(/[\s,]+/).map((w) => w.trim().replace(/[^a-z0-9]/g, "")).filter((w) => w.length > 2 && !exclude.includes(w));
      ...
      <div className="flex-grow max-w-4xl mx-auto w-full px-4 pb-20">
        ...
        <div className="text-center mb-10 mt-4">
          ...
          <p className="text-gray-500 text-base md:text-lg max-w-2xl mx-auto">
            Generate low competition hashtags for Instagram Reels, TikTok, and YouTube. Sorted by viral, medium, and niche tiers. Includes Urdu Romanized & Desi tags.
          </p>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-6 md:p-10 mb-8">
          ...
          <div className="mb-6">
            ...
            {currentPlatform && <p className="text-xs text-gray-400 mt-2">{currentPlatform.tip}</p>}
          </div>

          {/* Category Presets */}
          <div className="mb-6">
            <label className="block text-sm font-semibold text-gray-700 mb-3">Quick Category Presets</label>
            <div className="flex flex-wrap gap-2">
              {CATEGORIES.map((cat) => (
                <button key={cat.label} onClick={() => setKeywords(cat.keywords)} className="px-3 py-1.5 rounded-full border border-gray-200 text-sm text-gray-600 hover:border-sky-400 hover:text-sky-600 hover:bg-sky-50 transition-all">
                  {cat.label}
                </button>
              ))}
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            <div className="md:col-span-2">
              <label className="block text-sm font-semibold text-gray-700 mb-2">Keywords / Topic</label>
              <textarea value={keywords} onChange={(e) => setKeywords(e.target.value)} placeholder="e.g. karachi food, street eats, pakistani cuisine, iftar ideas" rows={3} className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-transparent text-gray-800 text-sm resize-y" />
            </div>
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">Number of Hashtags</label>
              <select value={numHashtags} onChange={(e) => setNumHashtags(Number(e.target.value))} className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-sky-500 bg-white text-gray-800">
                {[10, 15, 20, 25, 30, 50].filter((n) => n <= currentPlatform.maxTags * 2).map((n) => (<option key={n} value={n}>{n} hashtags</option>))}
              </select>
            </div>
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-2">Length Filter</label>
              <select value={maxLength} onChange={(e) => setMaxLength(e.target.value)} className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-sky-500 bg-white text-gray-800">
            ...
            <div>
              ...
              <select value={language} onChange={(e) => setLanguage(e.target.value)} className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-sky-500 bg-white text-gray-800">
            ...
            <div>
              ...
              <input type="text" value={excludeWords} onChange={(e) => setExcludeWords(e.target.value)} placeholder="e.g. spam, nsfw" className="w-full px-4 py-3 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-sky-500 text-gray-800 text-sm" />
            </div>
          </div>

          {/* Action Buttons */}
          <div className="grid grid-cols-2 gap-4 mb-6">
            <button onClick={generateHashtags} className="bg-sky-600 hover:bg-sky-700 text-white font-semibold py-3 rounded-xl transition-all shadow-sm hover:shadow-md flex items-center justify-center gap-2">
          ...
          {error && <div className="mb-4 p-3 bg-red-50 border border-red-200 rounded-lg flex items-start gap-2"><p className="text-red-600 text-sm">{error}</p></div>}
            ...
            <div className="mt-8">
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
                <div className="flex items-center gap-4 text-sm text-gray-500">
                  <span><strong className="text-gray-800">{charCount}</strong> / 2,200 chars {charCount > 2200 && <span className="text-red-500 ml-1">(over limit)</span>}</span>
                </div>
                <button onClick={copyAll} className="inline-flex items-center gap-2 px-5 py-2.5 bg-sky-600 hover:bg-sky-700 text-white rounded-xl text-sm font-medium transition-colors">
                  <Copy size={14} /> {copied === "all" ? "Copied!" : `Copy All ${hashtags.length}`}
                </button>
              </div>
              ...
              {[
                ...
                <div key={id} className={`border rounded-2xl p-5 mb-4 ${borderColor} ${bgColor}`}>
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <button onClick={() => copyGroup(id)} className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-medium transition-colors ${copyBg}`}>
                      <Copy size={12} /> {copied === id ? "Copied!" : "Copy Group"}
                    </button>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {tags.map((tag, i) => (
                      <button key={i} onClick={() => { navigator.clipboard.writeText(tag); }} className="px-3 py-1.5 bg-white border border-gray-200 hover:border-sky-400 rounded-full text-sm text-gray-700 hover:text-sky-600 transition-all hover:scale-105 active:scale-95" title="Click to copy">
                        {tag}
                      </button>
                    ))}
                  </div>
              ...
              {charCount > 2200 && (
                <div className="bg-red-50 border border-red-200 rounded-xl p-4 mt-2">
                  <p className="text-sm text-red-600"><strong>Instagram character limit exceeded.</strong> Your hashtags total {charCount} characters but Instagram caps at 2,200. Reduce the count or remove some long tags.</p>
                </div>
              )}
            </div>
          )}
        ...
        <section className="bg-white border border-gray-200 rounded-2xl p-6 md:p-10 mb-8">
          ...
          <ol className="space-y-5">
            {[
              { step: "1", title: "Pick Platform & Category", desc: "Select Instagram/TikTok and choose a preset like Food or Travel, or enter custom keywords." },
              { step: "2", title: "Set Language & Filters", desc: "Choose English, Urdu Romanized, or Mixed. Filter by length if needed." },
              { step: "3", title: "Generate & Copy", desc: "Click Generate. Copy High/Medium/Low tiers separately for a balanced mix." },
            ].map((item) => (
        ...
        <section className="bg-white border border-gray-200 rounded-2xl p-6 md:p-10 mb-8">
          ...
          <p className="text-gray-500 text-sm mb-6">It combines your keywords with trending modifiers. Here's the logic.</p>

          <div className="space-y-5">
            <div className="bg-gray-50 border border-gray-100 rounded-2xl p-5">
              <h3 className="font-bold text-gray-900 text-sm mb-2">Keyword Expansion</h3>
              ...
              <p className="text-gray-500 text-xs leading-relaxed">Combines your input keywords with popular modifiers like 'trending', 'viral', and '2026' to create high-volume tags.</p>
            </div>

            <div className="bg-gray-50 border border-gray-100 rounded-2xl p-5">
              <h3 className="font-bold text-gray-900 text-sm mb-2">Competition Tiering</h3>
              <div className="bg-gray-900 text-green-400 font-mono text-sm px-4 py-3 rounded-xl mb-3 overflow-x-auto">
                if (isShort || isTrending) return "high";
              </div>
              <p className="text-gray-500 text-xs leading-relaxed">Classifies tags based on length and popularity. Short/viral tags are High Comp, niche-specific ones are Low Comp.</p>
            </div>
            ...
            <div className="bg-sky-50 border border-sky-100 rounded-xl p-4">
              ...
              <p className="text-sky-800 text-xs leading-relaxed">
                All generation happens locally in your browser. No data is sent to any server. 100% private.
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="bg-white border border-gray-200 rounded-2xl p-6 md:p-10 mb-8">
          ...
          <p className="text-gray-500 text-sm mb-6">See how different locales produce different results.</p>

          <div className="space-y-5">
            <div className="border border-gray-100 rounded-2xl p-5">
              <div className="flex items-center gap-2 mb-3">
                <span className="text-sm bg-green-100 text-green-700 font-bold px-2.5 py-1 rounded-lg">English Locale</span>
              </div>
              <div className="bg-green-50 border border-green-100 rounded-lg p-4">
                <p className="font-mono text-sm text-gray-800 break-all">#Food #Foodie #KarachiFood #StreetFood #Recipe #Cooking #Iftar #Biryani #DesiFood #FoodLovers #InstaFood #Yummy #Delicious #FoodPorn #Chef #HomeCooking #FoodBlogger #Tasty #Eat #FoodStagram</p>
              </div>
            </div>

            <div className="border border-gray-100 rounded-2xl p-5">
              <div className="flex items-center gap-2 mb-3">
                <span className="text-sm bg-blue-100 text-blue-700 font-bold px-2.5 py-1 rounded-lg">Urdu Romanized Locale</span>
              </div>
              <div className="bg-blue-50 border border-blue-100 rounded-lg p-4">
                <p className="font-mono text-sm text-gray-800 break-all">#Dil #Pyar #Zindagi #Khushi #Desi #Pakistani #Subhanallah #Alhamdulillah #Mashallah #Dua #Sukoon #Junoon #Yaar #Maza #Swag #DesiVibes #PakistaniCulture #UrduPoetry #Shayari #Love</p>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className="bg-white border border-gray-200 rounded-2xl p-6 md:p-10 mb-8">
          ...
          <p className="text-gray-500 text-sm mb-6">Not just influencers. Here's where smart tagging matters.</p>
          <div className="grid sm:grid-cols-2 gap-4">
            {[
              { icon: <Zap size={20} className="text-sky-600" />, title: "Content Creators", desc: "Boost reach on Instagram Reels and TikTok by using the right mix of viral and niche tags." },
              { icon: <BarChart2 size={20} className="text-green-600" />, title: "Social Media Managers", desc: "Save hours of research by generating platform-specific tag sets for clients instantly." },
              { icon: <Globe size={20} className="text-violet-600" />, title: "Small Businesses", desc: "Get discovered by local customers using location-based and industry-specific hashtags." },
              { icon: <HelpCircle size={20} className="text-amber-600" />, title: "Influencers", desc: "Stay ahead of trends with auto-generated viral tags for maximum engagement." },
            ].map((item, i) => (
        ...
        <section className="bg-white border border-gray-200 rounded-2xl p-6 md:p-10 mb-8">
          ...
          <p className="text-gray-600 leading-relaxed">
            This tool runs 100% in your browser. No data is sent to any server. Your keyword inputs are never stored or tracked. Close the tab and it's gone. That's how it should be.
          </p>

          <p className="text-gray-600 leading-relaxed mt-4">
            Need emojis for your captions? Try the{" "}
            <Link href="/tools/emoji-picker" className="text-sky-600 underline underline-offset-2 hover:text-sky-700">Emoji Picker</Link>. Counting characters for bio limits? The{" "}
            <Link href="/tools/word-counter" className="text-sky-600 underline underline-offset-2 hover:text-sky-700">Word Counter</Link> has your back.
          </p>
        </section>
        ...
        <section className="bg-white border border-gray-200 rounded-2xl p-6 md:p-10 mb-8">
          ...
          <div className="space-y-3 max-w-4xl mx-auto">
            {[
              { q: "How to get more impressions on Instagram reels with hashtags?", a: "Use a mix of 5 high-competition viral hashtags, 15 medium-competition relevant hashtags, and 5-10 low-competition niche hashtags. Place them in the first comment, not the caption. Our tool automatically sorts your hashtags into these three tiers." },
              { q: "How many hashtags for Instagram reels to go viral in 2026?", a: "Instagram allows up to 30 hashtags per post. The best strategy in 2026 is using 25-30 hashtags with a mix of all three competition tiers. Avoid using only viral hashtags — niche tags help the algorithm understand your content." },
              { q: "What are low competition hashtags and why do they matter?", a: "Low competition hashtags have fewer posts but a highly targeted audience. When you use them, your post is much more likely to appear on the Explore page for that specific tag. They matter because they bring in views from people actually interested in your niche." },
              { q: "How to find trending hashtags for TikTok Pakistan?", a: "Enter your topic keywords in our tool, select TikTok as the platform, and choose Mixed or Urdu Romanized language style. The tool will generate hashtags including Pakistani trending tags like #tiktokpakistan, #desi, and niche-specific tags for your content." },
              { q: "Best hashtag strategy for small Instagram accounts?", a: "Small accounts should focus heavily on low and medium competition hashtags. Use only 3-5 high-competition tags and fill the rest with niche-specific tags. This maximizes the chance of appearing in search results for your specific niche." },
              { q: "Can I generate Urdu or Romanized Pakistani hashtags?", a: "Yes. Select Urdu Romanized or Mixed language style to include desi modifiers like #desi, #pakistani, #subhanallah, #zindagi, #pyar, #dil and more. These work well for Pakistani audience content on Instagram and TikTok." },
              { q: "Does this tool work for YouTube and Twitter?", a: "Yes. Select YouTube or Twitter/X from the platform dropdown. The tool adjusts the maximum number of hashtags recommended for each platform (15 for YouTube, 3 for Twitter) to ensure optimal performance." },
              { q: "Is this hashtag generator free?", a: "Yes, 100% free. No signup, no account, no limits. Generate as many hashtag sets as you need for your social media content." },
            ].map((item, i) => (
        ...
        <section>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[
              { href: "/tools/emoji-picker", title: "Emoji Picker", desc: "Search & copy emojis." },
              { href: "/tools/word-counter", title: "Word Counter", desc: "Count chars for bios." },
              { href: "/tools/midjourney-prompt-generator", title: "Midjourney Prompts", desc: "Create AI image prompts." },
              { href: "/tools/case-converter", title: "Case Converter", desc: "Change text case." },
